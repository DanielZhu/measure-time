...
exports = module.exports = function timeUsingMiddleware (opts) {
  return function timeUsingMiddleware (req, res, next) {
    ...
    if (matchedMeasure) {
      ...
      req.mtKey = formatDate + '.' + md5(matchedMeasure.pageName + '-' + req.pageStartTime + '-' + randomNum())
      var costTimeStart = {
        mtKey: req.mtKey,
  ...
  if (record.hasOwnProperty('mtKey')) {
    ...
    fse.readFile(logFilePath, function (err, savedCostTime) {
      if (err) {
        ...
        var matched = savedCostTime.hasOwnProperty('mtKey') && record.mtKey === savedCostTime.mtKey
        if (matched) {
...
function collectForPage (record) {
  ...
  fse.readFile(logFilePath, (err, savedPageLogs) => {
    if (err) return console.error(err)
    try {
